...
export async function execute(int, client) {
    ...
    if (roles.length === 0) {
        return int.editReply({ content: "No valid roles found to remove." });
    }

    // Get the user to add roles to (from the embed field)
    const userIdMatch = embed.fields[0].value.match(/\d{17,}/);
    if (!userIdMatch) {
        return int.editReply({ content: "Could not determine user to remove roles from. " });
    }
    ...
    await int.editReply({
        content: [
            removed.length ? `Removed roles: ${removed.map(r => `<@&${r}>`).join(", ")}` : "",
